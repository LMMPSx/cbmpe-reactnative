...
// Definindo o tipo de dados da tabela para uso nas funções de exportação
type TableRow = {
    id: number;
    prioridade: string;
    tipo: string;
...
/**
 * Gera o conteúdo no formato CSV (Comma Separated Values).
 */
const generateCsvContent = (data: TableRow[]): string => {
    const header = "id,prioridade,tipo,periodo,regiao,status\n";
    const rows = data.map(row => 
        // Usando aspas duplas em torno do campo período caso contenha vírgulas, embora não seja o caso aqui
        `${row.id},${row.prioridade},${row.tipo},"${row.periodo}",${row.regiao},${row.status}`
    ).join('\n');
    return header + rows;
};

/**
 * Simula a geração de conteúdo PDF (em formato de texto simples para visualização no console).
 */
const generatePdfReportText = (tableData: TableRow[]): string => {
    let report = "### Relatório de Ocorrências (PDF Simulado) ###\n\n";
    report += "RESUMO\n";
    report += "--------------------------------------\n";
    report += `Total de Ocorrências: 145\n`;
    report += `Tempo Médio de Resposta: 12min\n`;
    report += "--------------------------------------\n\n";
    
    report += "DADOS DA TABELA\n";
    report += "Data | Prioridade | Tipo | Região | Status\n";
    report += "------------------------------------------------\n";

    tableData.forEach(row => {
        report += `${row.periodo.padEnd(15)} | ${row.prioridade.padEnd(10)} | ${row.tipo.padEnd(10)} | ${row.regiao.padEnd(7)} | ${row.status}\n`;
    });
...
export default function ReportsScreen({ navigation }: any) {
    ...
    // --- DADOS PARA O GRÁFICO DE PIZZA (Por Tipo) --- (ENTIDADE MANTIDA)
    const dataPizza = [
        {
            name: "Incêndio",
            population: 45,
    ...
    // --- DADOS PARA O GRÁFICO DE BARRAS (Por Região) --- (ENTIDADE MANTIDA)
    const dataBarras = {
    ...
    // Configuração visual do Gráfico de Barras (ENTIDADE MANTIDA)
    ...
    // Dados da Tabela (ENTIDADE MANTIDA)
    ...
    // --- HANDLERS DE EXPORTAÇÃO (AGORA FUNCIONAIS) ---

    const handleGeneratePDF = () => {
        const pdfContent = generatePdfReportText(tableData);
        // Em um app real, o pdfContent (ou um HTML gerado) seria usado com 'expo-print' ou libs nativas.
        console.log("--- Conteúdo do PDF Gerado ---");
    ...
    const handleExportCSV = () => {
        ...
        // Em um app real, o csvContent seria salvo ou compartilhado com 'expo-sharing'.
        console.log("--- Conteúdo do CSV Gerado ---");
